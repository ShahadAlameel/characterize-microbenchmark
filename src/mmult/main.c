...
const int64_t SIZE_DATA = sizeof(float) * ((int64_t)m * (int64_t)p      // src1 matrix
                                        +  (int64_t)p * (int64_t)n      // src2 matrix
                                        +  (int64_t)m * (int64_t)n);    // result matri
...
int main(int argc, char** argv)
{
  ...
  //int scaler_r = 1000000;

  /* Data */
  int64_t data_size = (int64_t)m * (int64_t)n * sizeof(float);
  int b         = 32;
  ...
  //   /* Scale m,n,p by some scaler n to avoid segmentation error*/
  //  if (strcmp(argv[i], "-r") == 0 || strcmp(argv[i], "--scale") == 0) {
  //    assert (++i < argc);
  //    scaler_r = atoi(argv[i]);

  //    continue;
  //   }

  /*control the number of blocks in opt.c*/
...
//    printf("    -r | --scaler_r  Scale the random arrays size by taking the modulus of the array dimensions.(default = %d)\n", scaler_r);
    printf("    -b | --blocks    The number of elements processed in every iteration in the opt implementtion (default= %d)\n", b);
  ...
  byte* src1  = __ALLOC_INIT_DATA(byte, m * p * sizeof(float));
  byte* src2  = __ALLOC_INIT_DATA(byte, p * n * sizeof(float));
